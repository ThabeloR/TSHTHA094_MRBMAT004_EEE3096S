...
void pause_sec(float x)
{
	...
	//YOUR CODE HERE
	volatile int i,j,timer;
	for(timer = 1; timer <= x*10; timer++){
		for (i=0; i <= DELAY1; i++ ){
			for (j=0; j <= DELAY2; j++);
		}
	}
...
void getTime (void)
{
 ...
 int epochFromTime(TIME time){
	...
	sum = (((time.year - 22)*31536000 +(time.dayofmonth + day- 1)*86400+
					 time.hour*3600 + time.minutes*60 + time.seconds));
...
/**
  ...
  *         where the assert_param error has occurred.  c
